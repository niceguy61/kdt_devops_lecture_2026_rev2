# Week 1 Day 3: 배포, 실행 환경 표준화, Docker 진입 준비
...
오늘은 Docker를 깊게 배우기 전, Docker가 왜 필요한지 납득할 수 있는 문제를 먼저 만든다. 로컬 실행 환경 차이, 의존성 충돌, 빌드 산출물, 실행 명령, 로그, README, 설치 검증을 하나의 흐름으로 연결한다. AI Coding Tool 실습도 "코드를 만들어 달라"에서 끝내지 않고, 생성된 코드가 운영 가능한지 검토하는 관점으로 진행한다.
...
- Docker Desktop 설치와 `docker version`, `docker run hello-world` 검증을 수행한다.
...
- Docker Docs: `docker version` CLI reference  
...
오늘은 로컬 실습을 중심으로 진행한다. Docker Desktop 설치는 운영체제, 권한, 가상화 상태에 따라 시간이 걸릴 수 있다. 설치가 막히면 억지로 다음 단계로 넘어가지 않고 오류 메시지와 현재 상태를 기록한다.

필요한 준비:
- GitHub 로그인 가능
- VS Code 또는 선호 편집기 실행 가능
- 터미널에서 `git`, `python3`, `curl` 사용 가능
- Docker Desktop 설치 권한 또는 설치된 환경
- Docker Hub 또는 Docker Desktop 로그인 가능하면 확인
...
- `lesson-06.md`: Docker Desktop 설치 및 검증
- `lesson-07.md`: AI Coding Tool 준비
- `lesson-08.md`: AI Coding Tool 실습
- `mini-deploy-lab/`: 배포와 README 실습용 미니 앱
- `assets/`: 교안용 시각 자료

## Deliverables
- Docker 설치 또는 미설치 원인 기록
- `docker version` 또는 설치 실패 증거
- `docker run hello-world` 성공 결과 또는 실패 로그
...
- Docker Desktop 설치 상태를 명령어 결과로 증명할 수 있다.
