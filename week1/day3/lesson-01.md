...
아래 인포그래픽은 수동 배포 병목과 자동화된 배포 파이프라인의 차이를 한눈에 비교한다. 핵심은 특정 담당자에게 묶인 대기 시간을 줄이고, 개발자가 push한 변경이 정해진 검증 절차를 거쳐 빠르게 개발환경과 스테이징 환경에 반영되는 구조를 만드는 것이다.

![수동 배포 병목과 자동화 배포 사례](./assets/lesson-01-deployment-automation-case.png)
