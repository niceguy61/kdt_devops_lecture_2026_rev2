...
## 학습 제어
### 시작 3분 회상
- alert firing과 실제 장애 복구 완료는 어떻게 다른가?
- runbook에서 symptom→metric/kubectl→action→handoff 순서는 왜 필요한가?
### 오늘 반드시 가져갈 것
- runbook은 관찰 신호를 담당자 행동과 성공 기준으로 번역한다.
- handoff에는 재현 명령, 결과, 해석, 다음 책임이 있어야 한다.
### 최소 복구 경로
- 증상별 metric/log/event를 매핑한다 → 첫 확인 명령을 정한다 → 성공 기준과 handoff를 적는다.
- 성공 판정은 다른 사람이 같은 판단을 재현하는 것이다. 첫 실패는 runbook 첫 확인 위치에서 찾는다.
- 다음 lesson 진입 조건은 배움일기에 한 장애 runbook을 완성하는 것이다.