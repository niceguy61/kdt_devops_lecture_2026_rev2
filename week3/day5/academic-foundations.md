...
## Deployment, ReplicaSet, Scheduler count 구분
Deployment와 ReplicaSet은 둘 다 replica 수를 다룬다. 하지만 둘 다 기본적으로 node별 count가 아니다.

| 대상 | 무엇을 맞추는가 | node 배치와의 관계 |
|---|---|---|
| Deployment | application 배포 단위의 전체 desired replica | node를 직접 고르지 않음 |
| ReplicaSet | 특정 Pod template의 desired/current Pod 수 | node를 직접 고르지 않음 |
| Scheduler | 생성된 Pod를 어느 node에 둘지 결정 | node 선택 담당 |
| DaemonSet | 보통 node마다 Pod 1개씩 유지 | node 수와 직접 연결 |

수업에서는 다음 문장으로 고정해서 설명한다.

```text
Deployment는 "몇 개의 앱을 운영할지" 선언한다.
ReplicaSet은 "이 Pod 모양을 몇 개 유지할지" 맞춘다.
Scheduler는 "그 Pod를 어느 node에 둘지" 결정한다.
node마다 하나씩은 DaemonSet 계열의 문제다.
```

rolling update에서는 이 구조가 더 잘 보인다.

```text
Deployment desired replicas = 2
old ReplicaSet desired = 2 -> 0
new ReplicaSet desired = 0 -> 2
전체 Ready replica는 Deployment 기준으로 2를 향해 움직인다.
```
