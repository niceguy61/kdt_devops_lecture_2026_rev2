...
## 학습 제어
### 시작 3분 회상
- 서비스 간 통신에서 호출자 로그와 목적지 로그 중 무엇을 먼저 비교해야 하는가?
- API URL 오류와 DB_HOST 오류는 토폴로지에서 어느 경로를 끊는가?
### 오늘 반드시 가져갈 것
- 증상(호출 실패), 원인(잘못된 주소/중단된 dependency), 증거(logs/inspect)를 분리한다.
- 장애 전파는 첫 실패 서비스에서 시작되며 모든 downstream 오류가 원인은 아니다.
### 최소 복구 경로
- 정상 baseline과 장애 재현을 비교한다 → API URL/DB_HOST를 확인한다 → logs와 inspect로 첫 실패를 판정한다.
- 성공 판정은 증상·원인·증거를 세 칸으로 분리하는 것이다. 첫 실패 위치는 호출자 logs와 dependency 상태에서 확인한다.
- 다음 lesson 진입 조건은 복구 후 baseline을 다시 증명하는 것이다.