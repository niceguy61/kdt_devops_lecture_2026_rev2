...
## 학습 제어
### 시작 3분 회상
- 서비스 토폴로지에서 장애 범위와 호출 방향은 어떻게 연결되는가?
- Monolith와 MSA에서 데이터 책임과 배포 단위는 어떻게 달라지는가?
### 오늘 반드시 가져갈 것
- 분해의 기준은 서비스 개수가 아니라 배포·장애·데이터 책임의 경계다.
- MSA는 격리를 얻는 대신 네트워크와 운영 복잡성을 책임진다.
### 최소 복구 경로
- 같은 기능을 monolith/MSA 표로 비교한다 → 배포 단위와 장애 전파를 표시한다 → 각 선택의 운영 증거를 한 개씩 적는다.
- 성공 판정은 trade-off를 말할 수 있는 것이다. 첫 실패는 비교표의 책임 경계에서 확인한다.
- 다음 lesson 진입 조건은 서비스 분해가 운영 책임을 어떻게 바꾸는지 설명하는 것이다.