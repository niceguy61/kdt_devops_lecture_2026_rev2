...
## 학습 제어
### 시작 3분 회상
- 직전 Docker 실습에서 컨테이너와 서비스의 차이는 무엇이었나?
- 한 서비스 장애가 downstream에 전파되는 경로를 토폴로지로 그리면 어디서 시작되는가?
### 오늘 반드시 가져갈 것
- 서비스 토폴로지는 실행 목록이 아니라 호출 방향과 장애 전파 책임의 모델이다.
- 정상 상태의 증거와 장애 증거를 분리해야 원인을 좁힐 수 있다.
### 최소 복구 경로
- 서비스 목록과 호출 방향을 1분 안에 그린다 → 각 서비스의 health/log를 확인한다 → 호출 실패와 downstream 상태를 대조한다.
- 성공 판정은 호출 경로와 첫 실패 지점을 말할 수 있는 것이다. 첫 실패는 해당 서비스의 logs와 dependency 상태에서 확인한다.
- 다음 lesson 진입 조건은 서비스 간 책임과 장애 전파 방향을 한 문장으로 설명하는 것이다.