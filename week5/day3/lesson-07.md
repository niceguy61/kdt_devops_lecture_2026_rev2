...
```mermaid
flowchart LR
  Service["Service"] --> Metric["Metric namespace"]
  Metric --> Dimension["Dimension"]
  Dimension --> Graph["CloudWatch graph"]
  Graph --> Threshold["Threshold candidate"]
  Threshold --> Alarm["Alarm state"]
  Alarm --> Action["Operations action"]
```
