...
계정 권한, 비용, Docker Hub/ECR 인증 상태에 따라 ECS와 App Runner 중 하나를 실습 경로로 선택할 수 있다. 중요한 것은 특정 Console 화면을 외우는 것이 아니라, image -> service -> health -> logs -> update -> rollback의 운영 루프를 이해하는 것이다.
