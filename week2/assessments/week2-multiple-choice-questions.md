...
2. named volume 또는 필요한 bind mount를 사용하고, 삭제 전 위험을 README에 확인한다.
...
### 문제해설
컨테이너를 삭제하면 컨테이너 내부 writable layer의 데이터는 사라질 수 있다. 데이터 보존이 필요하면 volume을 사용하고, `docker volume rm` 같은 정리 명령의 위험도 함께 확인해야 한다.
...
### 문제 내용
`docker run -p 8080:80 nginx`를 실행했는데 port가 이미 사용 중이라는 오류가 발생했다. 가장 적절한 확인과 대응은 무엇인가?
...
2. port conflict 증상과 사용한 명령을 확인하고, 기존 process/container 확인 후 다른 host port로 재실행하고 HTTP 응답을 재확인한다.
3. Docker를 삭제한다.
...
### 출제의도
image tag가 공유와 재현의 기준이 되며, registry 사용 전 민감정보와 불필요한 파일 포함 여부를 확인해야 함을 확인한다.
...
### 문제해설
tag는 어떤 image를 공유하고 실행할지 식별하는 기준이다. push 전에는 이미지에 민감정보가 들어갔는지, README나 terminal 확인에 token/password가 노출되지 않았는지 확인해야 한다.

---

## 10. Week 2 학습 정리 학습 흐름

### 문제 내용
Week 2 학습 정리 학습 흐름에 포함하기 가장 적절한 내용 조합은 무엇인가?
...
2. build/run/compose 명령, HTTP/log/status 확인 지점, 겪은 문제와 재확인, 보안/정리 note, hands-on 발전 내역, Week 3 질문을 포함한다.
...
### 문제해설
Week 2 학습 흐름은 글의 화려함보다 재현 가능한 확인 지점과 판단 근거가 중요하다. 실행 명령, 확인 결과, 문제 해결 확인, 보안/정리 기준, 다음 주차로 이어지는 질문이 있으면 학습 상태를 확인하기 쉽다.
