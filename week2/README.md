...
## Overview
2주차는 Week 1에서 다룬 로컬 실행 조건을 Docker 이미지와 컨테이너로 옮기는 주간이다. 학생은 "내 컴퓨터에서는 실행된다"를 넘어서, 같은 실행 조건을 다른 사람도 재현할 수 있도록 Dockerfile, image, container, port binding, volume, environment variable, Compose로 이해한다.
...
Docker는 운영 문제를 모두 없애는 도구가 아니다. 실행 환경을 더 명시적으로 포장하고, 실행 조건을 문서와 명령으로 재현 가능하게 만드는 도구다. 따라서 이번 주는 명령어 암기보다 실행 조건, 흔한 실수, 장애 분석, 비용/보안/cleanup 기준을 중심으로 다룬다.

## 처음이면 여기부터
Week 2는 Docker가 반드시 필요하다. macOS는 Docker Desktop을 기본 경로로 진행하고, Linux는 Docker Engine 설치를 기본 경로로 진행한다. Docker Desktop for Linux는 조직 장비 정책상 필요한 경우의 예외 경로로만 분리한다. 설치가 끝난 뒤에는 "앱을 열었다"가 아니라 CLI 출력으로 상태를 확인한다.
...
- Linux에서 권한 문제가 나면 `sudo docker run --rm hello-world` 성공 여부와 Docker post-install 적용 여부를 구분한다.
...
- logs, inspect, exec, stats로 container 상태를 확인한다.
- Docker Compose로 유명한 로컬 아키텍처 패턴을 실행하고 검증한다.
- 로그, HTTP status, container status로 장애 원인을 분석한다.
...
강의 본문에는 분 단위 흐름표를 넣지 않는다. 학생용 자료는 개념 전개, 실습 명령, 확인 지점, 흔한 오해, 주의할 점, cleanup 기준을 중심으로 작성한다.
...
| Week 1 확인 지점 | Week 2 Docker 표현 | 주의할 점 |
|---|---|---|
...
| log 확인 | `docker logs`, `docker compose logs` | 장애 단서를 어디서 확인하는가 |
| README run step | Docker build/run/compose section | 다른 사람이 실행 절차를 재현할 수 있는가 |
| RCA 관점 | Docker failure analysis | port conflict, env missing, volume reset을 구분할 수 있는가 |
...
- PostgreSQL SQL 조작 흐름: user, database, table, insert, query
- storage/network 확인 지점
- image/Dockerfile/registry 확인 지점
- env/logs/inspect/exec failure drill 주의점
- Compose architecture 실행 흐름
- port, log, environment variable, volume, network 중 하나 이상을 포함한 장애 분석 흐름
...
- [ ] Week 1 로컬 PostgreSQL을 삭제/중지/보류 중 하나로 처리하고 이유를 구분했다.
...
- [ ] env/config 주입, logs/inspect/exec, secret 비노출 기준을 구분했다.
- [ ] `compose.yaml`로 최소 2개 아키텍처를 실행하고 검증했다.
- [ ] logs/status/HTTP response 중 하나 이상으로 정상 상태를 확인했다.
- [ ] port conflict, env missing, volume issue 중 하나를 장애 분석 흐름으로 설명할 수 있다.
